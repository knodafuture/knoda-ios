...
static BOOL SpacerEnabled = NO;

@implementation UINavigationItem (iOS7Spacing)

+ (void)setSpacerEnabled:(BOOL)spacerEnabled {
    SpacerEnabled = spacerEnabled;
}
- (BOOL)isIOS7
{
...
{
    if ([self isIOS7] && leftBarButtonItem && SpacerEnabled) {
...
{
    if ([self isIOS7] && leftBarButtonItems && leftBarButtonItems.count > 0 && SpacerEnabled) {
...
{
    if ([self isIOS7] && rightBarButtonItem && SpacerEnabled) {
...
{
    if ([self isIOS7] && rightBarButtonItems && rightBarButtonItems.count > 0 && SpacerEnabled) {
